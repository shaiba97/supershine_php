        ...
        $comment = $_POST['comment'];
            ...
            if (empty($first_name) && empty($last_name) && empty($phone) && empty($position) && empty($comment) && $fileSize === 0){
                ...
                if(empty($position)){
                    ...
                    header('Location: /supershine/admin/details/member.php?param='.$_GET['param']);
                    exit();
                }else if(!preg_match($alphabet, $position)){
                    $_SESSION['errors'] = ['type' => 'regex', 'msg' => 'Position must be only alphabets letter!'];
                    header('Location: /supershine/admin/details/member.php?param='.$_GET['param']);
                    exit();
                }else
                        ...
                        if(in_array($fileActualExt, $allowed)){
                            ...
                            if ($fileError === 0) {
                                ...
                                $fileNewName = uniqid('', true).".".$fileActualExt;
                                    ...
                                    if ($move) {
                                        $sql = "UPDATE members SET image=?, first_name=?, last_name=?, phone=?, position=?, comment=? WHERE id= ?";
                                        $stmt = $conn->prepare($sql);
                                        $stmt->bind_param("ssssssi", $fileNewName, $first_name, $last_name, $phone, $position, $comment, $id);
